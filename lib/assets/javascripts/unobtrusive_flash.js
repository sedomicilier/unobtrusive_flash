...
(function() {
  ...
  function nukeCookie(cookieName) {
      ...
      hostParts.forEach(function(i,part) {
  ...
  window.UnobtrusiveFlash.showFlash = function(flashMessages) {
    if (flashMessages !== null) {
      flashMessages.forEach(function(flashMessage) {
        var event = new CustomEvent('rails:flash', { detail: {type: flashMessage[0], message: flashMessage[1]}});
  ...
  document.addEventListener('page:change page:load', UnobtrusiveFlash.showFlashFromCookies); //TURBOLINK-CLASSIC
  document.addEventListener('turbolinks:load', UnobtrusiveFlash.showFlashFromCookies); //TURBOLINK (5)
